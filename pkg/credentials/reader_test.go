...
func TestReadDecryptsWhatTheEditorWrote(t *testing.T) {
	...
	yaml := "development:\n  name: from-file\n  max_retries: 3\n"
	if err := editor.EncryptAndSave([]byte(yaml), key); err != nil {
...
func TestLegacyFileRefusedUntilMigrated(t *testing.T) {
	...
	var cfg Config
	if err := NewConfigReader(dir).Read("development", &cfg); err == nil {
		t.Fatal("a legacy file was read without the opt-in")
	}

	reader := NewConfigReader(dir)
	reader.AllowLegacy = true
	if err := reader.Read("development", &cfg); err != nil {
		t.Fatalf("Read with AllowLegacy: %v", err)
	}
